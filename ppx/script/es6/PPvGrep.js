...
const dogrep = (argCmd === 'grep') ? gr : (argCmd === 'rg') ? rg : jv;
{
  ...
  const check_Mxxx = PPx.Extract('%*getcust(M_grepPPv)').split('\u000D\u000A');
  if (check_Mxxx.length === 3 || reload_opt === '1') {
    if (use_grep === true) {
      PPx.Execute(`*setcust M_grepPPv:grep=*string i,cmd=grep %%: *string i,gopt=${gr.lock}${gr.add} %%: \
      ...
      *string e,flen=${gr.lock.length} %%: *string e,blen=${gr.lock.length + gr.add.length}`);
      ...
      PPx.Execute('*deletecust "M_grepPPv:grep"');
    }
    if (use_ripgrep === true) {
      PPx.Execute(`*setcust M_grepPPv:rg=*string i,cmd=rg %%: *string i,gopt=${rg.lock}${rg.add} %%: \
      ...
      *string e,flen=${rg.lock.length} %%: *string e,blen=${rg.lock.length + rg.add.length}`);
      ...
      PPx.Execute('*deletecust "M_grepPPv:rg"');
    }
    if (use_jvgrep === true) {
      PPx.Execute(`*setcust M_grepPPv:jvgrep=*string i,cmd=jvgrep %%: *string i,gopt=${jv.lock}${jv.add} %%: \
      ...
      *string e,flen=${jv.lock.length} %%: *string e,blen${jv.lock.length + jv.add.length}`);
      ...
      PPx.Execute('*deletecust "M_grepPPv:jvgrep"');
    }
  }
}
...
const str = (esc => {
  try {
    ...
    return esc = PPx.Extract(`%*script(%'scr'%\\compCode.js,"iOs","""%%","${argCmd} %%si""gopt"" ※\\=\\\\\\\\", \
      "*string e,px=PPv %%%%: *execute %%%%%%%%M_grepPPv,!${argCmd}")`);
  } catch (e) {
