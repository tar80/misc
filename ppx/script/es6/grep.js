...
// 使用するコマンド
const use_LF_grep    = true;
const use_LF_rg      = true;
const use_LF_jvgrep  = true;
const use_PPv_grep   = true;
const use_PPv_rg     = true;
...
const rgLF = (use_LF_rg === true)
  ? { 'op': 'LF', 'lock': '-n --no-heading', 'add': ' --color never -Li -C1', 'complist': 'RGOPTION.TXT' } : null;
const rgPPv = (use_PPv_rg === true)
  ? { 'op': 'ppv', 'lock': '-n --no-heading', 'add': ' --color always -Li -C1', 'complist': 'RGOPTION.TXT' } : null;
// jvgrep
