...
var maskExt = [];
try {var ext = PPx.Arguments(0)} catch (e) {var ext = 'none'};
switch (PPx.Extract(PPx.WindowIDName)) {
  ...
  default:
    ...
    if (ext === 'image') {
      ...
      // PPxのエイリアス(ターミナル呼び出し)
      PPx.Execute('termppx');
      PPx.Execute('%0..\\mplayer\\mplayer.exe -framedrop -geometry %*windowrect(%N.,l):%*windowrect(%N.,t) -vf dsize=%*windowrect(%N.,w):%*windowrect(%N.,h):0 ' + fileName + ' -loop 0');
      PPx.Quit(1);
    };
    break;
};
